...
public class CancellationServiceImpl implements ICancellationService {
	...
	public Cancellation createCancellation(CancellationDTO dto) throws BookingNotFoundException, UserNotFoundException {
		...
		User cancelledBy = userRepository.findById(dto.getCancelled_by_user_id()).orElseThrow(
				() -> new UserNotFoundException("User not found with ID: " + dto.getCancelled_by_user_id()));

		Cancellation cancellation = new Cancellation();
		cancellation.setReason(dto.getReason());
		cancellation.setRefundAmount(dto.getRefundAmount());
		cancellation.setRefundStatus(dto.getRefundStatus());
	...
	@Override
	public Cancellation updateCancellation(Long cancellation_id, CancellationDTO dto)
			throws CancellationNotFoundException, UserNotFoundException, BookingNotFoundException {
		Cancellation cancellation = cancellationRepository.findById(cancellation_id).orElseThrow(
				() -> new CancellationNotFoundException("Cancellation not found with ID: " + cancellation_id));

		Booking booking = bookingRepository.findById(dto.getBooking_id())
				.orElseThrow(() -> new BookingNotFoundException("Booking not found with ID: " + dto.getBooking_id()));

		User cancelledByUser = userRepository.findById(dto.getCancelled_by_user_id()).orElseThrow(
				() -> new UserNotFoundException("User not found with ID: " + dto.getCancelled_by_user_id()));
	...
	public Cancellation getCancellationById(Long cancellation_id) throws CancellationNotFoundException {
		return cancellationRepository.findById(cancellation_id).orElseThrow(
				() -> new CancellationNotFoundException("Cancellation not found with ID: " + cancellation_id));
	}
